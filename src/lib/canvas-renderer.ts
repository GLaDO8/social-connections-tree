...
function drawCohortGroups(
  ...
  for (const c of cohorts) {
    ...
    // Tight bounding box around node edges (center ± radius), no extra padding
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    ...
    const isRelevant =
      !highlight.active ||
      !hoveredNodeId ||
      members.some((m) => m.id === hoveredNodeId);
...
function drawNodes(
  ...
  for (const person of persons) {
    ...
    const baseRadius = getVisualRadius(
      degree,
      maxDegree,
      person.isEgo,
      d.nodeRadius,
    ...
    if (isSelected) {
      ...
      ctx.arc(
        person.x,
        person.y,
        baseRadius + d.selectedGlowOffset,
        0,
        Math.PI * 2,
      );
    ...
    if (showLabel) {
      ...
      ctx.fillText(
        person.name,
        person.x,
        person.y + drawRadius + d.labelOffset,
      );
    }
