...
|------|---------|--------------|
| `graph-config.ts` | **Single source of truth** for all config: relationship classification (`getCategory`, `getBondStrength`), physics constants (`PHYSICS` incl. `egoLinkStrengthMultiplier`), bond distance/strength/radial tables, rendering constants (`DIMMED_ALPHA`, `BOND_OPACITY`, `FADE_DURATION`), interaction constants (`EDGE_HIT_THRESHOLD`), UI data (`RELATIONSHIP_TYPES`, `BOND_LABELS`, `DEFAULT_COHORT_COLORS`), `VisualSettings` interface + `VISUAL_DEFAULTS` (incl. `degreeAlphaMin`, `showEgoEdges`, `labelStrokeColor`, `labelStrokeWidth`, `cohortGroupPadding`) | Changing any visual, physics, or classification parameter |
| `graph-constants.ts` | Uniform node sizing (`getVisualRadius`, `computeDegreeStats`) and `stripPhysicsState` helper | Changing node size logic or saving/snapshotting |
...
1. **Link force** — Bond strength (1-5) controls both distance and rigidity. Strongest bonds (5) pull to 60px, weakest (1) to 300px. Configured via `BOND_DISTANCE` and `BOND_LINK_STRENGTH` tables. **Ego link attenuation:** links connected to the ego node have their strength multiplied by `PHYSICS.egoLinkStrengthMultiplier` (0.05), preventing the ego from pulling everyone inward too aggressively.

2. **Charge (many-body)** — Ego gets `PHYSICS.repulsion * 2` (−1400). Other nodes get `PHYSICS.repulsion * (1 + min(degree * 0.05, 0.5))`, so hub nodes repel slightly more. Capped at `chargeDistanceMax: 800`.

3. **Center** — Weak pull (`0.05`) toward viewport center. Keeps graph from drifting.

4. **Collide** — Radius = `getVisualRadius(0, 0, isEgo) + PHYSICS.collisionPadding`. Prevents node overlap. Strength `0.8`, runs 2 iterations per tick.

5. **Radial** — Concentric rings around ego based on bond strength to ego. Best friends at 150px ring, acquaintances at 700px (`BOND_RADIAL`: 5→150, 4→270, 3→400, 2→550, 1→700). Nodes not directly connected to ego placed at 500px. Ego pinned to center with strength 1.0, others at 0.15.

6. **Cluster** (`forceCohortCluster`) — Custom force that pulls each node toward the live centroid of its cohort(s). Multi-cohort nodes get pulled toward the average of their cohort centroids (Venn effect). Only active when a cohort has 2+ members. Strength: `0.20`.

7. **Exclusion** (`forceCohortExclusion`) — Custom force that pushes non-members out of foreign cohort areas. Computes each cohort's spatial radius (max member distance from centroid + 40px padding) and applies outward force on intruders proportional to their depth of penetration. Strength: `0.22`.

**Initial position seeding:** `createSimulation()` calls `seedInitialPositions()` before starting the simulation. Nodes are pre-positioned by cohort membership (angular sectors) and bond-to-ego distance (radial rings), with jitter to prevent overlap. Non-cohort nodes fill unclaimed angular sectors at a default radius of 500px. This prevents the simulation from starting as a tight hairball.
...
Uniform sizing via `getVisualRadius()` in `graph-constants.ts`. The function accepts `nodeRadius` and `egoRadius` parameters (driven by `VisualSettings`, defaults: 12 and 20):
- Ego: fixed `egoRadius` (default 20px)
- All other nodes: uniform `nodeRadius` (default 12px) — no degree-based scaling
...
5. Draw cohort group boundaries — rounded rectangles behind member nodes (bounding box expanded by `cohortGroupPadding`, default 25px), dimmed when hovering a non-member (only when `showCohortGroups` is enabled in `VisualSettings`). Fill opacity `0.08`, border opacity `0.25`, border width `1.5`, border radius `12`.
6. Draw edges — color by category, width/opacity by bond strength, bond-to-thickness enabled by default, dimming for non-highlighted. **Ego edge filtering:** ego edges hidden by default (`showEgoEdges: false`); they are only drawn when an endpoint is hovered/highlighted, and at 40% reduced opacity.
7. Draw nodes — color by first cohort (or default gray), uniform radius, hover expand + brighten, selection glow, cohort ring for active cohort, node border (if `nodeBorderWidth > 0`), fade-in animation for new nodes. **Degree-based alpha:** non-ego node fill alpha modulated by `degreeAlphaMin + (degree/maxDegree) * (1 - degreeAlphaMin)` (default `degreeAlphaMin: 0.45`), so low-degree nodes appear slightly transparent.
8. Draw labels — below nodes, LOD-gated at zoom < 0.4, always shown for highlighted nodes, hidden when `showLabels` is false. **Text halo:** `strokeText()` drawn before `fillText()` with `lineJoin="round"` using `labelStrokeColor` (#000000) and `labelStrokeWidth` (3) for readability against dark backgrounds. Default `labelSize: 13`.
